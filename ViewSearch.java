...
/**
 * ViewSearch class.
 * Implements the View interface. ONLY DISPLAYS RESULTS OF A #HASHTAG SEARCH. User search results direct
 * the current user to the profile of the searched user.
 */
public class ViewSearch implements View
{
	//DATA MEMBERS
	String searchInput;

	public ViewSearch(String input)
	{
		searchInput = input;
	}
	...
	{	
		...
		//Search for chirps that have that hashtag and count number	
		{
		...
		do {
			...
			else if (input == 'S')
			{
				...
				id = 0; //user id number
				int num = 0; //number of the post selected
				//IF THE TWEET THEY'RE VIEWING IS THEIR OWN, SET TO TRUE
	...
	{
		boolean reChirped = false;

		//Print number
		System.out.print(num + " ");
		//Print username
		//Find username according to id
		String username = " ";
		String origUser = " "; 
		System.out.print(username);
		if (reChirped)
		{
			System.out.print("(rechirped from " + origUser + ")");//???
		}
		//Print chirp text
		String chirpText = " ";
		System.out.println("  " + chirpText);
		//Print number of likes and rechirps
		int numLikes = 0;
		int numRechirps = 0;
		System.out.println("  " + "Likes: " + numLikes + " Rechirps: " + numRechirps);
	}	
